...
export class MainChartStockCandlesService {
  ...
  private _main_chart_stock_candles_result = new BehaviorSubject(this.data);
  readonly main_chart_stock_candles$ = this._main_chart_stock_candles_result.asObservable();

  ticker = "";

  get_values() {
    return this._main_chart_stock_candles_result.getValue();
  }

  reset_state() {
    this._main_chart_stock_candles_result.next(
      {
        "company_name": "",
        "data": true,
        "c": [],
  ...
  private set_values(val) {
    this._main_chart_stock_candles_result.next(val);
  }
  ...
  check_valid_data_present(ticker) {
    var cur_data = this._main_chart_stock_candles_result["_value"];
  ...
  get_company_stock_candles(ticker, resolution, from, to) {
    ...
    console.log(`MainChartStockCandlesService doesn't have data for the ticker: ${ticker}. Will fetch from backend and return.`)
    ...
    this.crud.get_company_stock_candles_data(ticker, resolution, from, to).subscribe(data => {
      ...
      console.log("MainChartStockCandlesService The result from get_company_stock_candles_data: " + JSON.stringify(data));
